    ...
    return (
        <div className="navbar">
            <nav className="navbarBig">
                ...
                {!currentUser && <div className="navButton" onClick={() => navigate('/login')}>登入</div>}
                {currentUser && <div className="navButton" onClick={() => navigate('/logout')}>登出</div>}
                {currentUser && <div className="navButton" onClick={() => navigate('/profile')}>個人檔案</div>}
                {currentUser && <div className="navButton" onClick={() => navigate('/reserve')}>預約琴房</div>}
                {currentUser && <div className="navButton" onClick={() => navigate('/list')}>琴譜列表</div>}
                {currentUser && currentUser === 'Admin' && <div className="navButton" onClick={() => navigate('/userList')}>使用者列表</div>}
                <div className="navButton" onClick={() => navigate('/news')}>最新消息</div>
                <div className="navButton" onClick={() => navigate('/')}>首頁</div>
            ...
            <nav className="navbarSmall">
                ...
                <div className="bigBox">
                    ...
                    <div className="bigDropbox" style={{ visibility: optionVisible }}>
                        {currentUser && <div className="brick" onClick={() => navigate('/profile')}>個人檔案</div>}
                        {currentUser && <div className="brick" onClick={() => navigate('/list')}>琴譜列表</div>}
                        {currentUser && currentUser === 'Admin' && <div className="brick" onClick={() => navigate('/userList')}>使用者列表</div>}
                        <div className="brick" onClick={() => navigate('/news')}>最新消息</div>
